...
# Remaining params
config.evalInstLevelScore = False
config.evalPixelAccuracy  = True
config.evalLabels         = []
config.printRow           = 5
...
# The main method
def evaluate_seg(predictionPath:str, groundTruthPath:str, label_split_file:str):
    global config
    groundtruth_splits = _read_imageset_file(label_split_file)
    predictionImgList = []
    groundTruthImgList = []

    if True :
        config.predictionPath = os.path.abspath(predictionPath)
        config.exportFile = os.path.join(config.predictionPath,"resultPixelLevelSemanticLabeling.json")
        if True :
            config.groundTruthSearch  = os.path.abspath(os.path.join(groundTruthPath,'*.png'))

        # use the ground truth search string specified above
        groundTruthImgList = sorted(os.listdir(groundTruthPath))
        groundTruthImgList = [
            os.path.join(groundTruthPath, image_name) for image_name in groundTruthImgList if image_name.split(".")[0] in groundtruth_splits
        ]
        ...
        predictionImgList = sorted(os.listdir(predictionPath))
        predictionImgList = [
            os.path.join(predictionPath, image_name) for image_name in predictionImgList
        ]

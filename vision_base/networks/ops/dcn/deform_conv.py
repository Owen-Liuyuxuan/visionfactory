...
try: 
    ...
    import warnings
    warnings.warn(
        f"Deformable Convolution Extension is not built from source. You will get error when using deformable conv with group number > 1."
    )
...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.groups == 1 and self.deformable_groups == 1:
            self.deform_conv_ops = self.tv_deform_conv2d_ops
        else:
            self.deform_conv_ops = self.cuda_deform_conv2d_ops
    ...
    def tv_deform_conv2d_ops(self, x, offset, mask):
        return torchvision.ops.deform_conv2d(
            input=x, offset=offset, weight=self.weight, bias=self.bias, stride=self.stride, padding=self.padding, dilation=self.dilation, mask=mask)

    def cuda_deform_conv2d_ops(self, x, offset, mask):
        return modulated_deform_conv(x, offset, mask, self.weight, self.bias,
                                    self.stride, self.padding, self.dilation,
                                    self.groups, self.deformable_groups)
    ...
    def forward(self, x):
        ...
        return self.deform_conv_ops(x, offset, mask)
...
if __name__ == '__main__':
    module = ModulatedDeformConvPack(3, 64, 3, padding=1).cuda()
